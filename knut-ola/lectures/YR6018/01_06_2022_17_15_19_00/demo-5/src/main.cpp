 ...
 * https://www.tinkercad.com/things/1VpFThyUO49
 */
void loop()
{
    if (button.getTimesPressed() >= 4)
    {
        if (timer.isFinished(20000))
        {
        ...
        {
            // Block the code below from running
            return;
        }
    }
    ...
    case 1:
        ...
        break;
    case 2:
        changeDetector.update(button.hasBeenReleasedFor(5000));
        if (changeDetector.changedToTrue())
        {
            blinkController.setBlinks(2);
        }
        break;
    case 3:
        // wasPressedThisCycle will be true
        // the same cycle as the getTimesPressed()
        // counts up to 3
        if (button.wasPressedThisCycle())
        {
            blinkController.setBlinks(1);
        }
        break;
    case 4:
        timer.reset();
        digitalWrite(greenLedPin, HIGH);
        // The rest of this case is handle by custom logic above
        break;
    }
}
